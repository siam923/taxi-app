...
class TaxiConsumer(AsyncJsonWebsocketConsumer):
    '''
    Note: When a message comes from a channel layer, Channels looks for 
    a function on the receiving consumer whose name matches the message 
    type. It also converts any . characters to _ characters before it 
    ...
    async def receive_json(self, content, **kwargs):
        '''On receiving data in websocket'''
    ...
    async def update_trip(self, message):
        '''
        On accepting rider's request update the trip:
           - let the rider know request has been accepted
           - Add the driver to the trip group
        '''
        data = message.get('data')
